...
export function InvoiceModal({ open, onOpenChange, invoice, onSuccess }: InvoiceModalProps) {
  ...
  const handleSubmit = (values: any) => {
    ...
    const payload = { ...values, invoiceDate: currentDate };
    if (isEditing) {
      mutation.mutate({ id: invoice.id, ...payload });
    } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 overflow-y-auto">
        ...
        <>
          <div className="mt-2">
            Invoice Number: <span className="text-gray-700 dark:text-gray-300"><b>{invoice?.invoiceNumber || ""}</b></span>
          </div>
          <Formik<InvoiceEditFormValues> 
            initialValues={{
